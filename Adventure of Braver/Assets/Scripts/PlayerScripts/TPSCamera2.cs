...
public class TPSCamera2 : MonoBehaviour {
	...
	void Start () {
		...
		if (Vector3.Cross (Vector3.right, Vector3.ProjectOnPlane (_tToCPoint, Vector3.up)).y < 0) {
			_angle = Vector3.Angle (Vector3.right, Vector3.ProjectOnPlane (_tToCPoint, Vector3.up));

		} else {
			_angle = -Vector3.Angle (Vector3.right, Vector3.ProjectOnPlane (_tToCPoint, Vector3.up));
		}
		Vector3 test = _targetTransform.TransformVector(Vector3.ProjectOnPlane (_tToCPoint, Vector3.up));;
		if (Vector3.Cross (Vector3.right, test).y < 0) {
			_angle = Vector3.Angle (Vector3.right, test);
	...
	void LateUpdate() {
		...
		if (Input.GetAxis ("CameraHorizontal") != 0) {
			...
			Vector3 rotationVector = _targetTransform.TransformVector(Vector3.ProjectOnPlane (_tToCPoint, Vector3.up));	//回転するベクトル
			Debug.LogError(rotationVector);
			//Debug.LogError(rotationVector2);
			...
			transform.position = centerOfRotation + _targetTransform.InverseTransformVector(rotationVector);
			//transform.position = Vector3.MoveTowards( transform.position,  point + rotationVector, 1f );	//これでも出来る
			//------------------------------------------------------------------------------------------------------------------------------

			//カメラをターゲットに向かせる----------------------------------------------------------------------------------------------------
			transform.forward = -_targetTransform.InverseTransformVector(rotationVector);
			//transform.forward = -Vector3.ProjectOnPlane (_tToCPoint, Vector3.up);
